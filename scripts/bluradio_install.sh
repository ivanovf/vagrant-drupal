...
# Nombre del profile
drupal_profile="bluradio_profile"
# Ruta predeterminada de instalación de Drupal
public_directory=/var/www/bluradio
...
sudo chmod 777 -R /var/www/bluradio/; sudo rm -rf /var/www/bluradio/*; sudo rm -rf /var/www/bluradio/.*; cd /var/www/bluradio

# Ruta de instalación de drupal
cd $public_directory

echo -e "${BOLD}2 de 7 ==> Clonando el repositorio del profile...${NONE}"
git clone git@bitbucket.org:icckmodules/bluradio_profile.git -b master ./

# Drush make del profile
echo "${BOLD}3 de 7 ==> Ejecutando drush make al profile '$drupal_profile'...${NONE}"
drush make bluradio_profile_stub.make --working-copy --force-complete --y
echo

echo -e "${BOLD}4 de 7 ==> Copiando settings file...${NONE}"
cp /home/vagrant/files/bluradio/settings.php sites/default/settings.php
...
# Drush make del profile
  git clone git@bitbucket.org:icckmodules/configuration-bluradio.git $profile_configurations_path
echo
...
if [ -d "$search_api_path" ]; then
  if [ ! -d "/opt/solr/solr-5.3.1/server/solr/$drupal_profile" ]; then
    sudo mkdir /opt/solr/solr-5.3.2/server/solr/$drupal_profile
    sudo mkdir /opt/solr/solr-5.3.2/server/solr/$drupal_profile/conf
  fi
  sudo cp $search_api_path/* /opt/solr/solr-5.3.2/server/solr/$drupal_profile/conf/
